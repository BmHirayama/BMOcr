...
// MARK: - Protocol
protocol OnPluginManagerDelegate {
    // [errorCode]
    // 0: 成功
    // -1: 失敗
    // -2: 権限エラー
...
class PluginManager: DriverCardOCRDelegate {
    ...
    // MARK: - Member
    private var mDelegate: OnPluginManagerDelegate? = nil
    ...
    func didScaned(result: [(String, String)], scanType: DriverCardOCRLibrary.SCAN_TYPE) {
        ...
        switch scanType {
            ...
            info.mCardType = PluginManager.CARDTYPE_MENKYOSYO
            ...
            case SCAN_TYPE.ZairyuCard:
                info.mCardType = PluginManager.CARDTYPE_ZAIRYU
                result.forEach { set in
                    if (set.0.compare("氏名") == .orderedSame) {
                        info.mName = set.1
                    }
            ...
            case SCAN_TYPE.MyNumberCard:
                info.mCardType = PluginManager.CARDTYPE_MYNUMBER
                var address1 = ""
                var address2 = ""
                result.forEach { set in
                    if (set.0.compare("氏名") == .orderedSame) {
                ...
                break
            default:
                mDelegate?.onResult(sender: self, code: PluginManager.CODE_FAILED, result: nil)
                return
        }
        mDelegate?.onResult(sender: self, code: PluginManager.CODE_SUCCESS, result: info)
    }
    ...
    // MARK: - Accesser
    func start(parent: UIViewController, delegate: OnPluginManagerDelegate?) {
        mDelegate = delegate
