...
public class PluginManager {
    ...
    // MARK: - Member
    private OnPluginManagerListener mListener = null;

    // MARK: - Listener
    public interface OnPluginManagerListener {
        void onResult(PluginManager sender, int code, PluginManager.OcrResultInfo result);
    }

    // MARK: - Accesser
    public void start (AppCompatActivity activity, OnPluginManagerListener listener) {
        mListener = listener;
        Permission.checkPermission(activity, new OnPermissionListener() {
            ...
            public void onResult(boolean result) {
                if (result) {
                    ...
                    if (mListener != null) {
                        mListener.onResult(PluginManager.this, CODE_AUTHROIZE, null);
                    }
                }
            }
        });
                ...
                if (result == DriverCardOCR.RESULT.SUCCESS) {
                    if (mListener != null) {
                        ...
                        if (scanType == DriverCardOCR.SCAN_TYPE.DriverCard) {
                            ...
                            mListener.onResult(PluginManager.this, CODE_FAILED, null);
                            return null;
                        }
                        mListener.onResult(PluginManager.this, CODE_SUCCESS, info);
